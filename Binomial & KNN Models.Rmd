...
## Logistic Regression 5% Graphs & Confusion Matrix

```{r log 5pct graphs and matrix, include=TRUE}
library(ROCR)
...
## Logistic Regression 10% Graphs & Confusion Matrix

```{r log 10pct graphs and matrix, include=TRUE}
library(ROCR)
m_pred <- prediction(binlog_5pct_probabilities, 
...
## Logistic Regression 20% Model

```{r 20pct model, include=TRUE}
model_binlog_20pct <- glm(data = df_train, family="binomial"(link='logit'),
  Change_20_Pct_Units_Sold_SD_Mil ~ 
    factor(Category) * Recession_Full_Year + 
    Year * 
    Recession_Full_Year +
    Recession_Pct_Year +
...
summary(model_binlog_20pct)
      
stargazer(model_binlog_20pct, type="text")
                  
# obtain odds ratio coefficients for better interpretation
exp(coef(model_binlog_20pct))
print(exp(cbind(Odds_Ratio = coef(model_binlog_20pct), 
                confint(model_binlog_20pct, level=0.90))), 
      digits=3)

    
vif(model_binlog_20pct)
#boxplot(df$Units_Sold_Diff_Mil ~ df$Recession_Full_Year)

binlog_20pct_probabilities <- predict(model_binlog_20pct, df_test, type = "response")

```

## Logistic Regression 20% Graphs & Confusion Matrix

```{r log 20pct graphs and matrix, include=TRUE}
library(ROCR)
m_pred <- prediction(binlog_20pct_probabilities, 
                     df_test$Change_20_Pct_Units_Sold_Diff_Mil)
m_perf <- performance(m_pred, "tpr", "fpr")
# ROC Curve
plot(m_perf, col = "blue", lwd = 2, main = "ROC Curve")
abline(a = 0, b = 1, col = "red", lty = 2)
# alt ROC Curve also showing the Youden Index
library(ROCit)
plot(rocit(score=binlog_20pct_probabilities,
           class=df_test$Change_20_Pct_Units_Sold_Diff_Mil))

# cutpoint for confusion matrix 
  # cut at 0.25 being close to the Youden Index
binlog_20_pred01 <- ifelse(binlog_20pct_probabilities > 0.25, 1, 0)

# view confusion matrix & model performance
library(caret)
caret::confusionMatrix(data=factor(binlog_20_pred01), 
                reference=factor(df_test$Change_20_Pct_Units_Sold_Diff_Mil ))
```
...
for (i in 1:krows) {
  ...
  change.knn <- knn(train = knn_df_train_IV, 
                    test = knn_df_test_IV, 
                    cl = df_train$Change_20_Pct_Units_Sold_SD_Mil, 
                    k=i)
  k.results[i,"RMSE"] <- (mean((change.knn != df_test$Change_20_Pct_Units_Sold_SD_Mil)^2))^0.5
}
